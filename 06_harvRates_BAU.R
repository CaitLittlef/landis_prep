...
data <- data %>% rename(b.over.rem = c.over.rem) 
...
# UPDATE 201022: NOT dividing by intervening years, as it's the same window across all treatments.
# Prop of plots is fine AS annual rate.

data <- data %>%
  mutate(prop.plots = plot.cnt/plot.ttl.mu) %>%
  # mutate(ann.rate = prop.plots / (yr.last - yr.first + 1)) %>% #+1 for yrs inclusive
  mutate(ann.rate = prop.plots) %>% # still calling this ann.rate so I don't have to fix code below.
...
# ^ NO!! Do not do the above, because it squashes county-specific rates. INCLUDE county.

data <- data %>%
  group_by(county, for.reclass, own.reclass, harv.type, tho.area.avg, tho.area.se) %>%
  mutate(ann.rate.avg = mean(ann.rate)) %>%
  mutate(hri = round(ifelse(ann.rate.avg == 0,NA,(1/ann.rate.avg)),0)) %>%
  ungroup() %>%
  dplyr::select(-plot.cnt, -plot.ttl.mu, -prop.plots, -ann.rate)
...
write.csv(data, paste0(out.dir, "parametersXcountyXforXown_", currentDate, ".csv"))
